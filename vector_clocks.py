...
def status_report(process_table):
        print('\nSTATUS\nEVENT       P_A   P_B   T_1   T_2')
        for status in process_table:
                data = status[1]
                print(str(status[0]).ljust(12),str(data[0]).ljust(5), str(data[1]).ljust(5),str(data[2]).ljust(5),str(data[3]).ljust(5))
...
internals = []
process_table = []
process_time = [[0,0,0] for _ in range(3)]
...
print('\nPLOT CLOCK SYNCHRONIZATION IN DISTRIBUTED SYSTEMS\n')
